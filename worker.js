...
async function dailyTasks(env) {
  ...
  for (const k of vips.keys) {
    ...
    if (hrsLeft > 20 && hrsLeft < 28) {
      try { await fetch("https://api.telegram.org/bot" + env.BOT_TOKEN + "/sendMessage", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: parseInt(uid), text: "⏰ Your VIP expires in ~24 hours.\nRenew to stay 💎" }) }); } catch (e) {}
    }
  }
  ...
  for (const k of vvips.keys) {
    ...
    const hrsLeft = (parseInt(expiry) - Date.now()) / 3600000;
    if (hrsLeft > 20 && hrsLeft < 28) {
      try { await fetch("https://api.telegram.org/bot" + env.BOT_TOKEN + "/sendMessage", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: parseInt(uid), text: "⏰ Your VVIP expires in ~24 hours.\nRenew to keep elite access 👑" }) }); } catch (e) {}
    }
  }
}
...
async function handleUpdate(request, env) {
  ...
  // ============ CHECK SPECIAL MODES BEFORE SHIELD ============
  ...
  // ============ SHIELD: BAN CHECK (skip in special modes) ============
  if (!isAdmin) {
  ...
  if (!isAdmin && text === "/start") {
    ...
    if (streakData.isNew && streakData.reward) {
      ...
      if (streakData.reward === "7-day-vip") rewardMsg = "🎉 7-DAY STREAK! FREE 1-day VIP 💎";
      if (streakData.reward === "14-day-vvip") rewardMsg = "👑 14-DAY STREAK! FREE 1-day VVIP!";
  ...
  if (text === "✖️ CANCEL") {
    ...
    await sendMsg(chatId, "Cancelled.", isAdmin ? adminKb : userKb);
    return new Response("OK");
  }
  ...
  if (supportMode === "yes" && !isAdmin && text) {
    ...
    // Save for admin inbox view
    await KV.put("support:" + Date.now() + ":" + userId, JSON.stringify({ name: firstName, uid: userId, text, time: Date.now() }), { expirationTtl: 604800 });
    await sendMsg(chatId, "✔️ Your message has been sent to admin. You'll receive a reply soon!", userKb);
    return new Response("OK");
  }
  ...
  if (teamMode === "yes" && !isAdmin && text) {
    ...
    await sendMsg(chatId, "✔️ You are now following: " + text + "\nYou'll get notified when they play!", userKb);
    return new Response("OK");
  }
  ...
  if (quizMode === "yes" && !isAdmin && text) {
    await KV.delete("quizmode:" + userId);
    const quizData = await KV.get("dailyquiz");
    if (!quizData) { await sendMsg(chatId, "Quiz expired. Try again tomorrow.", userKb); return new Response("OK"); }
  ...
  if (text === "/start") { const g = getTimeGreeting(); reply = isAdmin ? g + ", Boss 👑" : g + ", " + firstName + "!\nWelcome to VortexPulse AI."; }
  ...
  else if (text === "📩 USER MESSAGES" && isAdmin) {
    const messages = await KV.list({ prefix: "support:" });
    let r = "📩 SUPPORT INBOX\n━━━━━━━━━━\n";
    if (messages.keys.length === 0) r += "No messages.";
    else {
      const sorted = messages.keys.slice(-10).reverse();
      for (const k of sorted) {
        const d = JSON.parse(await KV.get(k.name));
        const ago = Math.floor((Date.now() - d.time) / 60000);
        r += "👤 " + d.name + " (ID: " + d.uid + ") - " + ago + "m ago\n💬 " + d.text.substring(0, 100) + "\n\nReply: /reply " + d.uid + " message\n━━━━━━━━━━\n";
      }
    }
    reply = r;
  }
  else if (text === "💰 REVENUE" && isAdmin) {
    const vipCount = parseInt(await KV.get("paid_vip_count") || "0");
    const vvipCount = parseInt(await KV.get("paid_vvip_count") || "0");
    const vipRev = vipCount * parseInt(vipWeekly);
    const vvipRev = vvipCount * parseInt(vvipWeekly);
    const total = vipRev + vvipRev;
    reply = "💰 REVENUE TRACKER\n━━━━━━━━━━━━\n💎 VIP subscriptions: " + vipCount + "\n👑 VVIP subscriptions: " + vvipCount + "\n\n💵 Est. VIP revenue: ₦" + vipRev.toLocaleString() + "\n💵 Est. VVIP revenue: ₦" + vvipRev.toLocaleString() + "\n━━━━━━━━━━━━\n💰 TOTAL: ₦" + total.toLocaleString() + "\n\n(Based on weekly rate)";
  }
  ...
  else if (text === "🎯 MY STREAK") { const data = await KV.get("streak:" + userId); if (!data) reply = "🎯 No streak. Use bot daily!"; else { const p = JSON.parse(data); const next = p.streak < 3 ? "3 days = bonus" : p.streak < 7 ? "7 days = FREE 1-day VIP" : p.streak < 14 ? "14 days = FREE 1-day VVIP" : "Legend!"; reply = "🎯 YOUR STREAK\n━━━━━━━━━━\n🔥 Current: " + p.streak + "\n🎁 Next: " + next; } }
  ...
  else if (text === "🔔 FOLLOW TEAM") { const fav = await KV.get("favteam:" + userId); await KV.put("teammode:" + userId, "yes", { expirationTtl: 300 }); reply = "🔔 Type your favourite team name:\n(e.g., Chelsea, Arsenal, Real Madrid)\n\n" + (fav ? "Currently following: " + fav : "Not following any team yet."); keyboard = cancelKb; }
  else if (text === "🎲 DAILY QUIZ") {
    const quizData = await KV.get("dailyquiz");
    if (!quizData) reply = "🎲 No quiz today. Check back tomorrow!";
    else {
      const claimed = await KV.get("quizclaim:" + userId);
      if (claimed) reply = "🎲 You already answered today's quiz!\nYour answer: " + claimed + "\nCome back tomorrow.";
      else { const q = JSON.parse(quizData); await KV.put("quizmode:" + userId, "yes", { expirationTtl: 600 }); reply = "🎲 DAILY QUIZ\n━━━━━━━━━━━━\n" + q.q + "\n\nType your answer now:\n(You earn 10 points)"; keyboard = cancelKb; }
    }
  }
    ...
    if (!games) { reply = "Fetch live games first."; keyboard = vvipMainKb; }
      ...
      const aiPick = await askGroq("From:\n" + games + "\nSafest " + text + ". Target " + target + ". Format with flag, league, time, match, pick, odds.", "Elite tipster.");
  ...
  else if (text === "ℹ️ HELP") reply = await askGroq("Brief friendly help for VortexPulse AI. Mention: Free Tips, VIP, VVIP, AI Chat, Lucky Pick, Quiz, Streak, Codes, Contact Admin. Under 80 words.");
    ...
    if (!pending) { reply = "Choose market first."; keyboard = isAdmin ? userKbAdmin : userKb; }
      ...
      if (gameList.keys.length === 0) { reply = "No games for " + market + "."; keyboard = tier === "free" ? freeKb : vipKb; }
        ...
        const aiPick = await askGroq("From:\n" + allGames + "\n" + fmt + " ONLY for: " + market + ". If not available, say so.", "Tipster.");
        const w = parseInt(await KV.get("wins:" + userId) || "0"); await KV.put("wins:" + userId, (w + 1).toString());
