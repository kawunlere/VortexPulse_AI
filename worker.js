export default {
  ...
  async scheduled(event, env, ctx) {
    // Daily auto-broadcast at 7 AM UTC
    await dailyBroadcast(env);
...
async function dailyBroadcast(env) {
  ...
  const greetings = [
    ...
    "New day, new wins! Today's predictions are warming up. Be ready ⚡"
  ];
...
async function cleanupExpired(env) {
  ...
  // Games auto-expire via TTL — just notify admin
  const free = await KV.list({ prefix: "game_free:" });
  const vip = await KV.list({ prefix: "game_vip:" });
  await fetch("https://api.telegram.org/bot" + env.BOT_TOKEN + "/sendMessage", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ chat_id: ADMIN_ID, text: "Daily Report 📊\nFree Games: " + free.keys.length + "\nVIP Games: " + vip.keys.length + "\n\nExpired games auto-cleaned. Brain is fresh 🧠" })
  });
}
...
async function handleUpdate(request, env) {
  ...
  const adminKb = [
    ...
    ["💬 ADMIN AI CHAT", "▪️ BROADCAST"],
    ["✔️ POST WINNINGS", "◼️ BOT STATS"],
  ...
  const userKb = [
    ...
    ["🏆 GAME OF THE DAY", "📊 MY WINS"],
    ["🎁 REFER FRIENDS", "🏅 LEADERBOARD"],
    ["👤 MY ACCOUNT", "ℹ️ HELP"],
    ["⬛ SUBSCRIBE VIP"]
  ];
  ...
  function getTimeGreeting() {
    const hr = new Date().getUTCHours() + 1; // Nigeria is UTC+1
  ...
  if (hasPhoto) {
    if (isAdmin) {
      ...
      if (adminMode === "upload_free" || adminMode === "upload_vip") {
        ...
        await sendMsg(chatId, "Analysing screenshot with AI Vision... Detecting market type 🧠", adminKb);
        const fileId = msg.photo[msg.photo.length - 1].file_id;
        const smartPrompt = "Look at this betting screenshot carefully. Identify EXACTLY which betting market(s) are shown (e.g., 1X2/Straight Win, Over/Under 1.5, BTTS, Correct Score, Corners, etc.). Then extract: each match (home team vs away team), league/country, and the specific odds shown for ONLY those visible markets. Don't invent markets that aren't visible. Format clearly with sections per match. End with a single line: 'MARKETS_DETECTED: [list of markets]'";
        ...
        if (extractedText) {
          ...
          const preview = extractedText.substring(0, 600);
      ...
      if (adminMode === "post_winning") {
        ...
        for (const key of users.keys) {
          ...
          try {
            ...
            await sendMsg(parseInt(uid), "🏆 Another WIN from VortexPulse AI! Subscribe VIP for more 💎");
            count++;
          } catch (e) {}
        }
  ...
  if (bcMode === "yes" && isAdmin && text !== "◀️ BACK") {
    ...
    await sendMsg(chatId, "✔️ Broadcast sent to " + count + " users.", adminKb);
    return new Response("OK");
  }
  ...
  if (inAdminChat && isAdmin && text !== "✖️ EXIT ADMIN CHAT") {
    const sysAdmin = "You are VortexPulse AI's brain assistant. You are talking to your OWNER/BOSS (admin). Be respectful, smart, and execute his orders. He can ask you to manage games, give insights, suggest improvements, or chat freely. Always recognise him as your Boss.";
  ...
  if (text === "✖️ EXIT ADMIN CHAT" && isAdmin) {
    ...
    await sendMsg(chatId, "Admin AI Chat ended.", adminKb);
    return new Response("OK");
  }
  ...
  if (isAdmin) {
    if (text.startsWith("/setpayment ")) {
      ...
      await sendMsg(chatId, "✔️ Payment updated.", adminKb);
      return new Response("OK");
    }
  ...
  if (text === "/start") {
    ...
    if (isAdmin) {
      ...
      reply = greet + ", " + firstName + "!\nWelcome to VortexPulse AI. I analyse markets to find the safest games for you. Please select an option below.";
    }
  }
  else if (text.startsWith("/start ref_")) {
    const refBy = text.replace("/start ref_", "").trim();
    await KV.put("refby:" + userId, refBy);
    reply = "Welcome! You were referred. Subscribe VIP and your friend earns a free day 🎁";
  }
    ...
    reply = "BOT STATS\n━━━━━━━━━━\n👥 Total Users: " + users.keys.length + "\n💎 Active VIPs: " + vips.keys.length + "\n🆓 Free Games: " + free.keys.length + "\n💎 VIP Games: " + vipG.keys.length;
  }
    ...
    if (free.keys.length === 0) reply = "Game of the Day not ready yet. Check back soon 🎯";
      ...
      const pick = await askGroq("From this game data:\n" + game + "\nPick the SAFEST single bet with team names and confidence %. Be brief and confident.", "You are a professional tipster.");
      reply = "🏆 GAME OF THE DAY\n━━━━━━━━━━\n" + pick;
    }
  }
  else if (text === "📊 MY WINS") {
    const wins = await KV.get("wins:" + userId) || "0";
    reply = "📊 Your Stats\n━━━━━━━━━━\nTotal picks viewed: " + wins + "\nWin tracker coming soon!";
  }
  else if (text === "🎁 REFER FRIENDS") {
    const botUsername = "your_bot_username"; // Replace later if needed
    reply = "🎁 REFERRAL PROGRAM\n━━━━━━━━━━\nShare this link:\nhttps://t.me/" + botUsername + "?start=ref_" + userId + "\n\nRefer 3 friends who subscribe = 1 day FREE VIP for you!";
  }
    ...
    const helpPrompt = "Generate a brief friendly help guide for VortexPulse AI bot. Mention: Free Tips, VIP Section, AI Chat, Subscribe VIP, Referral, Game of the Day. Keep it under 80 words.";
    reply = await askGroq(helpPrompt);
  }
    ...
    if (!pending) { reply = "Please choose a market first."; keyboard = isAdmin ? userKbAdmin : userKb; }
    else {
      ...
      if (gameList.keys.length === 0) {
        reply = "No games loaded yet for " + market + ".\nCheck back shortly — brain is updating.";
        ...
        const aiPrompt = "From these betting screenshots:\n\n" + allGames + "\n\nGenerate " + pickFormat + " ONLY for the market: " + market + ". If this market is not in the data, say 'No games available for this market right now'. Otherwise format clearly with teams, pick, confidence %, and total odds if slip. Be brief.";
        await sendMsg(chatId, "Analysing safe odds for " + market + "... 🧠\nPlease wait while I scan all games.", isAdmin ? userKbAdmin : userKb);
        const aiPick = await askGroq(aiPrompt, "You are VortexPulse AI, a professional tipster. Only use data provided. Never invent games.");
        // Increment user wins counter
        const currentWins = parseInt(await KV.get("wins:" + userId) || "0");
        await KV.put("wins:" + userId, (currentWins + 1).toString());
